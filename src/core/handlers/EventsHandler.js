...
module.exports = async (client) => {
  const { log } = client;
  (await pGlob(`${process.cwd()}/src/client/events/*/*.js`)).forEach((eventFile) => {
    const event = require(eventFile);

    if(!event.type == "custom" && !eventList.includes(event.name)) return log.warn(`Evenement non-chargé: nom invalide\nFichier -> ${eventFile}`);
